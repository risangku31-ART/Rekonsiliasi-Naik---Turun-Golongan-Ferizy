...
if st.button("🚀 Proses"):
    ...
    # Kandidat kolom
    C_INV_KEY = ["nomor invoice","no invoice","invoice"]
    ...
    C_TS_KEY  = ["nomor invoice","no invoice","invoice"]
    ...
    def first_idx(hdr: List[str], cands: List[str]) -> Optional[int]:
        return pick_col_idx(hdr, cands)
    ...
    for f in inv_files:
        ...
        for row in iter_records_xlsb(f, sheet_index=sheet_idx, scan_all_sheets=scan_all):
            if key_idx is None:
                hdr = list(row.keys())
                key_idx = first_idx(hdr, C_INV_KEY)
                amt_idx = first_idx(hdr, C_INV_AMT)
                tgl_idx = first_idx(hdr, C_INV_TGL)
                pay_idx = first_idx(hdr, C_INV_PAY)
                tuj_idx = first_idx(hdr, C_INV_TUJ)
                ch_idx  = first_idx(hdr, C_INV_CH)
                mer_idx = first_idx(hdr, C_INV_MER)
                if key_idx is None or amt_idx is None:
                    st.error(f"Header tidak lengkap di file Invoice: {f.name}")
                    break
    ...
    for f in ts_files:
        ...
        for row in iter_records_xlsb(f, sheet_index=sheet_idx, scan_all_sheets=scan_all):
            if key_idx is None:
                hdr = list(row.keys())
                key_idx  = first_idx(hdr, C_TS_KEY)
                amt_idx  = first_idx(hdr, C_TS_AMT)
                kode_idx = first_idx(hdr, C_TS_KODE)
                tkt_idx  = first_idx(hdr, C_TS_TKT)
                pay_idx  = first_idx(hdr, C_TS_PAY)
                gol_idx  = first_idx(hdr, C_TS_GOL)
                asal_idx = first_idx(hdr, C_TS_ASAL)
                ctk_idx  = first_idx(hdr, C_TS_CTK)
                if key_idx is None or amt_idx is None:
                    st.error(f"Header tidak lengkap di file T-Summary: {f.name}")
                    break
            vals = list(row.values())
            key = coerce_key(vals[key_idx])
            if not key: continue
    ...
    st.download_button(
        ...
        data=xlsx,
        file_name="rekonsiliasi_preview.xlsx",
